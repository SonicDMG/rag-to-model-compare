...
export interface AggregatedChartData {
  ...
  rag: RAGMetrics | null;
  /** Hybrid approach metrics (uses direct context with RAG data when needed) */
  hybrid: ApproachMetrics | null;
  /** Direct approach metrics (via Ollama) */
...
/**
 * Aggregate Hybrid metrics from query history
 */
function aggregateHybridMetrics(items: QueryHistoryItem[]): ApproachMetrics | null {
  const hybridResults = items
    .map(item => item.hybridResult as DirectResult)
    .filter(result => result && result.metrics);

  if (hybridResults.length === 0) return null;

  const times = hybridResults.map(r => r.metrics.generationTime);
  const tokens = hybridResults.map(r => r.metrics.tokens);
  const costs = hybridResults.map(r => r.metrics.cost);
  const contextUsages = hybridResults
    .map(r => r.metrics.contextWindowUsage)
    .filter((usage): usage is number => usage !== undefined);
  ...
  return {
    ...
    avgContextUsage: contextUsages.length > 0
      ? contextUsages.reduce((sum, u) => sum + u, 0) / contextUsages.length
      : 0,
    queryCount: hybridResults.length,
  };
}

/**
 * Aggregate Direct metrics from query history (via Ollama)
 */
function aggregateDirectMetrics(items: QueryHistoryItem[]): ApproachMetrics | null {
  const directResults = items
    .map(item => item.directResult as OllamaResult)
    .filter(result => result && result.metrics);
  ...
  const costs = directResults.map(r => r.metrics.cost); // Always $0
...
export function aggregateChartData(history: QueryHistoryItem[]): AggregatedChartData {
  ...
  return {
    ...
    hybrid: aggregateHybridMetrics(history),
...
export function getMaxValue(
  ...
  if (data.hybrid) values.push(data.hybrid[metric]);
