 ...
 * 
 * Dropdown selector for choosing between available Ollama models.
 * Displays model information including context window and family.
 */
...
export interface OllamaModelSelectorProps {
  ...
  currentModel: string;
  /** Array of available Ollama model names */
 ...
 * OllamaModelSelector Component
 * 
 * A dropdown selector for choosing between different Ollama models.
 * Displays model information including name, context window, and family.
 */
export function OllamaModelSelector({
  ...
  const currentModelConfig = getOllamaModelConfig(currentModel);
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {/* Model Label */}
      <label htmlFor="ollama-model-selector" className="text-sm font-medium text-unkey-gray-200">
        Inference Model:
      </label>
        ...
        >
          {availableModels.length === 0 ? (
            ...
            availableModels.map((modelName) => {
              ...
              const contextWindowK = config 
                ? (config.contextWindow / 1000).toFixed(0) 
                : '?';
              const family = config?.family || 'unknown';

              return (
                <option key={modelName} value={modelName} className="bg-unkey-gray-850">
                  {modelName} ({contextWindowK}K context, {family})
                </option>
              );
            })
          )}
      ...
      {currentModelConfig && (
        <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-unkey-gray-800/50 border border-unkey-gray-700 rounded-md">
          ...
          <span className="text-xs text-unkey-gray-500">•</span>
          <span className="text-xs text-unkey-gray-400">
            {currentModelConfig.family}
          </span>
        </div>
      )}
    </div>
